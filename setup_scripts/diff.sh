...
# - - - - - - - - - - - - - - - -
    ...
    diff .config/herbstluftwm/autostart  ~/.config/herbstluftwm/autostart  >  "$DIFFS_FILE"/config_herbstconfig
    diff .config/nvim/init.vim           ~/.config/nvim/init.vim           >  "$DIFFS_FILE"/config_nvimrc
    diff .config/picom/picom.conf        ~/.config/picom/picom.conf        >  "$DIFFS_FILE"/config_picom
...
# - - - - - - - - - - - - - - - -
    ...
    [[ -z $(ls -A1 "$DIFFS_FILE") ]] && rm -rf "$DIFFS_FILE" || exit
